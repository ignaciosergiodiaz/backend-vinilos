      ...
      if (!albumRemoved) {
        ...
        Song.find({ album: albumRemoved._id }).remove((err, songRemoved) => {
          ...
          if (err) {
            ...
            if (!songRemoved) {
              res.status(404).send({ message: 'El album  no ha sido eliminada' });
            } else {
...
function uploadImage(req, res) {
  ...
  if (req.files) {
...

    {
      ...
      Album.findByIdAndUpdate(artistId, {image: file_name}, (err, albumUpdated) => {

        if(!albumUpdated){
          res.status(404).send({ message: 'No se ha podido actualizar la imagen del audio' });
        } else {
